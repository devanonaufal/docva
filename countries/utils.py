...
def get_profile_photo(size: tuple = (280, 340), person_id: str = None, gender: str = "Random") -> Image.Image:
    ...
    try:
        # Foto directory: override via DOCVA_FOTO_DIR, else bundled with the package
        foto_dir = Path(os.environ.get("DOCVA_FOTO_DIR", "")) if os.environ.get("DOCVA_FOTO_DIR") else Path(__file__).parent / "foto"
